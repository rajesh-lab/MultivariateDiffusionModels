...
from mdm.score_models.biggg_model.models import utils, layers, layerspp, normalization
...
class NCSNpp(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.act = get_act(config.nonlinearity)

        self.image_channels = config.in_channels * config.n_vars
        self.image_size = config.height
        
        self.aux_vars = True if config.n_vars > 1 else False
        ...
        self.all_resolutions = [self.image_size //
            ...
            if self.progressive != 'none':
                if i_level == self.num_resolutions - 1:
                    if self.progressive == 'output_skip':
                        ...
                        modules.append(
                            conv3x3(in_ch, self.image_channels, init_scale=init_scale))
                        pyramid_ch = self.image_channels
                    elif self.progressive == 'residual':
                        modules.append(nn.GroupNorm(num_groups=min(
                            in_ch // 4, 32), num_channels=in_ch, eps=1e-6))
                    ...
                    if self.progressive == 'output_skip':
                        modules.append(nn.GroupNorm(num_groups=min(
                            in_ch // 4, 32), num_channels=in_ch, eps=1e-6))
                        modules.append(
                            conv3x3(in_ch, self.image_channels, bias=True, init_scale=init_scale))
                        pyramid_ch = channels
                    elif self.progressive == 'residual':
    ...
    def forward(self, input, t):
        ...
        input = input.view(batch_size, self.image_channels, self.image_size, self.image_size)
