...
class HoUNet(nn.Module):
    def __init__(
        ...
        self.mat_dims = (
            self.config.in_channels * self.config.n_vars,
            self.config.width,
            self.config.height,
        )

        self.input_channels = self.config.in_channels * self.config.n_vars
        self.input_height = self.config.width
        self.ch = ch
        self.output_channels = self.config.out_channels * self.config.n_vars
    ...
    def forward(self, x, temp):
        # Init
        x = x.view(x.shape[0], *self.mat_dims)
        B, C, H, W = x.size()
